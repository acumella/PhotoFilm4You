...
public class AddProductMBean implements Serializable{
	...
	public AddProductMBean() throws Exception
	{
		newProduct = new ProductJPA();
		name = null;
		description = null;
		dailyPrice = 0;
		result = "AddProductMBean called";
	}
	...
	public List<String> getModelNames() {
		...
		try {
			...
			for (Iterator<ModelJPA> iter2 = modelsList.iterator(); iter2.hasNext();)
			{
				...
				if(model2.getBrand().getName().equalsIgnoreCase(this.brandName) || this.brandName == null)
	...
	{	
		try {
			if(name != null) {
				...
				result = myCatalogRemote.addProduct(this.name, this.description, categoryId, modelId, brandId,  this.dailyPrice);
